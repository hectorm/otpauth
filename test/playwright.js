...
(async () => {
	...
	await page.addScriptTag({ path: require.resolve('chai/chai.js') });
	...
	page.exposeFunction('exitTest', async (code) => { browser.close(); process.exit(code); });
	await page.evaluate(() => mocha.run((code) => setTimeout(() => { exitTest(code); }, 10)));
